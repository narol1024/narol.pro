// forked from /node_modules/hexo/lib/plugins/helper/list_categories.js, but it added prefix option
...
function listCategoriesHelper(categories, options) {
  ...
  const { style = 'list', transform, separator = ', ', prefix = '', suffix = '' } = options;
  ...
  const hierarchicalList = (level, parent) => {
    ...
    prepareQuery(parent).forEach((cat, i) => {
      ...
      result += `<a class="${className}-list-link${isCurrent ? ' current' : ''}" href="${prefix}${url_for.call(this, cat.path)}${suffix}">`;
  ...
  const flatList = (level, parent) => {
    ...
    prepareQuery(parent).forEach((cat, i) => {
      ...
      result += `<a class="${className}-link" href="${prefix}${url_for.call(this, cat.path)}${suffix}">`;
      result += transform ? transform(cat.name) : cat.name;
...
hexo.extend.helper.register('list_categories_plus', listCategoriesHelper);